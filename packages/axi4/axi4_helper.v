// ============================================================
// AXI4 Formal Helper — Verilog
// Provides:
//   - Per-channel handshake timeout flags
//   - Write beat counter (tracks WLAST vs AWLEN)
...
module axi4_helper #(
  ...
  output reg  [7:0]        snap_awlen,
  output reg  [7:0]        cnt_w_beats,   // beats sent in current write burst
  ...
  // ============================================================
  // Write beat counter — detect WLAST mismatch vs AWLEN
  // ============================================================
  // Latch AWLEN at AW handshake; count W beats; flag if WLAST
  // fires on the wrong beat.
  always @(posedge ACLK or negedge ARESETn) begin
    if (!ARESETn) begin
      snap_awlen      <= '0;
      cnt_w_beats     <= '0;
